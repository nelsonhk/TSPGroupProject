...
class TSPSolver:
    ...
    def greedy(self, time_allowance=60.0):
        cities = self._scenario.getCities()
        best_cost = float('inf')
        best_solution = TSPSolution(cities)

        # start the timer for the algorithm
        start_time = time.time()

        # loop through all the nodes in the graph
        # let each node be the starting node, calculate greedy path, choose the shortest path
        count = 0
        while count < len(cities) and time.time() - start_time < time_allowance:
            start = cities[count]
            current = start
            path_distance = 0
            visited = [current]
